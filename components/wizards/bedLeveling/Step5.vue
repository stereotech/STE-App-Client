...
export default class extends Vue {
  ...
  async performStep () {
    if (this.additionalData.size === PrinterSize.Large) {
      await this.customCommand({ id: this.settings.systemId, command: 'G0 X280 Y20 F3600' })
    }
    else {
      await this.customCommand({ id: this.settings.systemId, command: 'G0 X180 Y20 F3600' })
    }
